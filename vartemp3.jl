...
function analyze_parameter_estimation_problem(PEP::ParameterEstimationProblem; test_mode=false)
    ...
    if (test_mode)
        println("test result is", besterror < 1e-2)
    end
...
function varied_estimation_main()
    ...
    for PEP in [
        ...
        #daisy_ex3(datasize, time_interval, Tsit5()),  #Vern9() gives a weird error here.  ideally isolate it and create an issue.  also, estimation is inaccurate

...
@testset "Run larger, slower parameter recovery tests on known ODEs" begin
	...
	function analyze_parameter_estimation_problem(PEP::ParameterEstimationProblem; test_mode = false)
		...
		res2 = SPEWrapper(PEP.model, PEP.measured_quantities,
			PEP.data_sample,
			PEP.solver, res[1])
		SPE_estimates = estimates = vcat(collect(values(res2.states)), collect(values(res2.parameters)))
		SPE_error = maximum(abs.((SPE_estimates .- all_params) ./ (all_params)))
		println("For model ", PEP.Name, ": The SPE  max abs rel. err: ", SPE_error)
